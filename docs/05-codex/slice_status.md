...
- Slice 19–23: planned local tracking MVP with custom exercises, set logging, analytics foundations, settings, onboarding.
...
| 2026-05-28 | 18 — Exercise analytics use cases | done | this commit | Added pure-Dart exercise analytics read models and a bounded WorkoutSet timeline use case for per-exercise set count, repetitions, volume, average kg/rep, estimated 1RM, previous comparable-session deltas, and equal-length time-window deltas. Hardened analytics formula identity and estimated 1RM validation deterministically. | `git status --short`; `git rev-parse --short HEAD`; `flutter pub get`; `flutter gen-l10n`; `dart run build_runner build --delete-conflicting-outputs`; `dart format --output=none --set-exit-if-changed .`; `flutter analyze`; `flutter test`; `scripts/check.sh`; requested analytics/no-remote/domain-purity guardrail `rg` checks. | Slice 19 owns charts and range-selector UI. |
